...
## ✅ T2. Edge --app= 应用模式打开选项 — 已实现于 v0.5.0（待验证/发布）

> 实现说明（简化点）：应用窗口复用采用「标题匹配 + UIA 无 Tab 控件」检测（同时覆盖 PWA 窗口——PWA 窗口同样无标签栏）；未实现 `shell:AppsFolder` 的 PWA AppID 提取启动（首次用 `--app=` 等价，复用仍可靠），需要时再补。
> 附带修正：窗口枚举统一为 **EnumWindows 全量**（每进程所有顶层窗口），浏览器标签模式在多窗口场景也能正确定位。
...
## ⬜ T5. 主窗口 Title 位置版本显示 + 更新高亮点击 — 已实现于 v0.3.0（方案 A：WindowChrome 自定义标题栏）✅
...
## ⬜ T6. 设置「打开软件时同时启动 dsh」— 已实现于 v0.3.0 ✅
...
## ✅ T7. 绑定主机 0.0.0.0（局域网共享）自动补丁挂载 — 已实现于 v0.4.0
...
## ✅ T8. dsh 来源设置（源码仓库 / 全局命令）— 已实现于 v0.5.0（待验证/发布）
...
- ✅ T2（Edge 应用模式打开）— 已实现于 v0.5.0（待验证/发布）
...
- ✅ T8（dsh 来源设置：源码 / 全局命令）— 已实现于 v0.5.0（含局域网地址虚拟网卡过滤修正，待验证/发布）
