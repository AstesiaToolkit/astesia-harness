...
- **打开方式可选**：浏览器标签页（已打开时 UIA 切换已有标签，含最小化/后台窗口自动恢复，多窗口全量枚举定位）或 Edge 应用窗口（`--app=` 独立无地址栏，已打开时聚焦复用，无 Edge 自动回退）；
- **智能打开浏览器**：DSH 运行中按钮始终可用；
- **关闭行为可选**：「关闭主界面时」可设为 退出程序 / 最小化到托盘（互斥），并可开启「退出时提示」——每次点 × 询问，勾选「不再提示」后写回设置持久记住；
- **设置自动保存**：设置修改后自动持久化，无需点「保存设置」——勾选框/单选/下拉改动即时落盘，文本输入（仓库路径/端口/附加参数）停止输入约 0.8 秒或失焦时保存；「恢复默认」立即生效；
...
- **dsh 来源可选**：源码仓库（`pnpm dsh web`，需仓库检出+依赖+dist）或全局命令（`npm i -g @deepseek-ai/dsh`，免源码），设置页实时显示当前 dsh 来源版本；
- **局域网共享**：绑定主机可选「0.0.0.0（局域网共享）」，自动生成补丁（经补丁层绕过 dsh CLI 守卫、端口跟随设置），状态区显示真实可达的局域网地址（自动过滤 Radmin VPN/Tailscale/Hyper-V 等虚拟网卡），带安全确认与常驻红色横幅；
...
1. 打开主窗口，在「设置」页确认 DSH 仓库路径与端口（默认 `F:\Codes\DownloadProjects\deepseek-harness` / `3080`）；设置修改后自动保存，无需手动保存；
2. 回「运行」页点「▶ 启动」；
